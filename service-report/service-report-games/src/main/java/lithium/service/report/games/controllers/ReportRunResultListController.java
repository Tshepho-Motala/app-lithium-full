...
import lithium.service.report.games.data.specifications.ReportRunResultSpecifications;
import lithium.tokens.LithiumTokenUtil;
import lombok.extern.slf4j.Slf4j;

@RestController
@RequestMapping("/report/games/{reportId}/runs/{reportRunId}/results")
@Slf4j
public class ReportRunResultListController {
	...
	public DataTableResponse<ReportRunResults> table(@PathVariable("reportRunId") ReportRun reportRun, DataTableRequest request, Principal principal) throws Exception {
		...
		if (!tokenUtil.hasRole(reportRun.getReport().getDomainName(), "REPORT_GAMES")) throw new AccessDeniedException("User does not have access to reports for this domain");
