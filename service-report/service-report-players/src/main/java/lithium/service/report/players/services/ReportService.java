...
public class ReportService {
	...
	public void addActionsAndActionLabelValues(ReportRevision rev, ReportActionBasic[] actions) {
		for (int i = 0; i < actions.length; i++) {
			...
			if (actions[i].getNotificationName() != null &&
					!actions[i].getNotificationName().isEmpty()) {
				String label = "";
				if (reportAction.getActionType().equalsIgnoreCase(
						ReportActionService.REPORT_ACTION_SEND_NOTIFICATION_TO_PLAYER)) {
					label = ReportActionService.LABEL_REPORT_PLAYER_NOTIFICATION_NAME;
				}
				if (label != null && !label.isEmpty()) {
					reportActionLabelValueService.findOrCreate(reportAction, label, actions[i].getNotificationName());
				}
			}
		}
	}
	...
	public void checkPermission(String domainName, Principal principal) {
		...
		if (!tokenUtil.hasRole(domainName, "REPORT_PLAYERS"))
