...
import lithium.service.affiliate.data.specifications.ReportRunResultSpecifications;
...
@RestController
@RequestMapping("/report/ia/{reportId}/runs/{reportRunId}/results")
@Slf4j
public class ReportRunResultListController {
	
	@Autowired
	ReportRunResultsRepository repo;
	...
	public DataTableResponse<ReportRunResults> table(@PathVariable("reportRunId") ReportRun reportRun, DataTableRequest request, Principal principal) throws Exception {
		...
		if (!tokenUtil.hasRole(reportRun.getReport().getDomainName(), "REPORT_IA")) throw new AccessDeniedException("User does not have access to income access reports for this domain");
