...
public class ProviderConfigService {
    ...
    public ProviderConfig getConfig(String providerName, String domainName) throws Status500ProviderNotConfiguredException {
        ...
        for(ProviderProperty p: pp.getData()) {
            ...
            if(p.getName().equalsIgnoreCase(ProviderConfigProperties.BET_HISTORY_ROUND_DETAIL_URL.getValue())) config.setBetHistoryRoundDetailUrl(p.getValue());
            if(p.getName().equalsIgnoreCase(ProviderConfigProperties.BET_HISTORY_ROUND_DETAIL_PROVIDER_ID.getValue())) config.setBetHistoryRoundDetailPid(p.getValue());
        }
