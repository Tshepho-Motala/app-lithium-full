...
import lithium.service.casino.provider.nucleus.data.request.AwardBonusRequest;
...
import lithium.service.casino.provider.nucleus.data.requestresponse.AwardBonusRequestResponse;
import lithium.service.casino.provider.nucleus.data.requestresponse.CancelBonusRequestResponse;
import lithium.service.casino.provider.nucleus.data.requestresponse.CheckBonusRequestResponse;
import lithium.service.casino.provider.nucleus.data.requestresponse.GetBonusInfoRequestResponse;
import lithium.service.casino.provider.nucleus.data.response.AwardBonusResponse;
...
//			}
//		}
		restTemplate.setInterceptors(Arrays.asList(new NucleusXmlInterceptor()));
		return restTemplate;
	}
	...
	public @ResponseBody lithium.service.casino.client.objects.response.AwardBonusResponse awardBonus(
		...
		BrandsConfigurationBrand bc = nucleusService.getBrandConfiguration(moduleInfo.getModuleName(), inRequest.getDomainName());
		...
		if (!(response.getResponse().getResult().equalsIgnoreCase(AwardBonusResponse.RESPONSE_SUCCESS) || response.getResponse().getResult().equalsIgnoreCase(AwardBonusResponse.RESPONSE_CODE_BONUS_CODE_ALREADY_EXISTS))) {
			...
			try {
				nucleusService.getMailService().save(SystemEmailData.builder()
			...
			nucleusService.retryFrbAllocation(request, bc, getRestTemplate(), FRB_AWARD_BONUS_URL_EXT, inRequest.getDomainName());
		}
	...
	public class NucleusXmlInterceptor implements ClientHttpRequestInterceptor {
		@Override
