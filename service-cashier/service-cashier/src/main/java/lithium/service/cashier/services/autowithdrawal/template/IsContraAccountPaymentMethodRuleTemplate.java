...
import static lithium.service.cashier.client.objects.enums.AutoWithdrawalRuleType.IS_CONTRA_ACCOUNT_PAYMENT_METHOD;

@Service
@Slf4j
public class IsContraAccountPaymentMethodRuleTemplate extends AutoWithdrawalRuleTemplate {
    ...
    public ValidatedResult validate(AutoWithdrawalRule rule, RuleValidateContext context) {
        try {
            ...
            return validateRule(rule, String.valueOf(paymentMethod.getContraAccount()));
        } catch (Exception e) {
            log.error("Unable to lookup if payment method has contra account while trying to"
    ...
    public AutoWithdrawalRuleType getType() {
        return IS_CONTRA_ACCOUNT_PAYMENT_METHOD;
    }
