...
import static lithium.service.cashier.client.objects.enums.AutoWithdrawalRuleType.LT_WITHDRAWALS_AMOUNT_IN_CENTS;

@Service
@Slf4j
public class LtWithdrawalsAmountInCentsRuleTemplate extends AutoWithdrawalRuleTemplate {
    @Autowired
    ...
    public ValidatedResult validate(AutoWithdrawalRule rule, RuleValidateContext context) {
        ...
        try {
            ...
            long lifetimeWithdrawalsAmount = accountingClientService.findLifetimeWithdrawalsAmountInCentsByUserAndGranularityAndCurrency(
                    user.getDomain().getName(), user.guid(), Granularity.GRANULARITY_TOTAL.granularity(), currency);
            return validateRule(rule, String.valueOf(lifetimeWithdrawalsAmount));
        } catch (Exception e) {
            log.error("Unable to lookup user's lifetime withdrawals amount while trying to"
    ...
    public AutoWithdrawalRuleType getType() {
        return LT_WITHDRAWALS_AMOUNT_IN_CENTS;
    }
