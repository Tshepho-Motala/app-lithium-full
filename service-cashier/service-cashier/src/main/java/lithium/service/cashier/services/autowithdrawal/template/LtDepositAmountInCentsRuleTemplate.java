...
import static lithium.service.cashier.client.objects.enums.AutoWithdrawalRuleType.LT_DEPOSIT_AMOUNT_IN_CENTS;

@Service
@Slf4j
public class LtDepositAmountInCentsRuleTemplate extends AutoWithdrawalRuleTemplate {
	...
	public ValidatedResult validate(AutoWithdrawalRule rule, RuleValidateContext context) {
		...
		try {
			...
			long lifetimeDepositAmount = accountingClientService.findLifetimeDepositAmountInCentsByUserAndGranularityAndCurrency(
					user.getDomain().getName(), user.guid(), Granularity.GRANULARITY_TOTAL.granularity(), currency);
			return validateRule(rule, String.valueOf(lifetimeDepositAmount));
		} catch (Exception e) {
			log.error("Unable to lookup user's lifetime deposit amount while trying to"
	...
	public AutoWithdrawalRuleType getType() {
		return LT_DEPOSIT_AMOUNT_IN_CENTS;
	}
}
