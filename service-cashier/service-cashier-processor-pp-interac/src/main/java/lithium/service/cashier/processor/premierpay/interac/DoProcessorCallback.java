...
public class DoProcessorCallback extends DoProcessorCallbackAdapter {
	...
	public DoProcessorCallbackResponse callback(DoProcessorCallbackRequest request, DoProcessorCallbackResponse response) throws DoErrorException, Exception {
		...
		Response<DoProcessorRequest> doProcessorRequest = getDoProcessorRequest(receivedTransactionId, extTransactionId, request.getProcessorCode(), true);
		...
		if (tranAction == Action.PENDING) {
			response.setStatus(DoProcessorResponseStatus.NOOP);
		} else if (tranAction == Action.PAYMENT) {
			switch (status) {
				case APPROVED:
		...
		response.setRedirect(configurationProperties.getGatewayPublicUrl()+"/service-cashier/frontend/closepage");
