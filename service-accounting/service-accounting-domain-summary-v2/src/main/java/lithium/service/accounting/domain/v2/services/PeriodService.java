...
import lithium.service.accounting.domain.v2.storage.repositories.PeriodRepository;
...
public class PeriodService {
	...
	private Period findOrCreatePeriodPerLifetime(Domain domain) {
		return findOrCreatePeriod(null, null, null, null, null, null, domain,
	...
	private Period findOrCreatePeriodPerWeek(Domain domain, int year, int week) {
		DateTime dateStart = new DateTime(year, 1, 1, 0, 0)
				.withWeekyear(year).withWeekOfWeekyear(week).withDayOfWeek(1);
		DateTime dateEnd = dateStart.plusWeeks(1);
		return findOrCreatePeriod(year, null, null, week, dateStart, dateEnd, domain,
	...
	private Period findOrCreatePeriodPerMonth(Domain domain, int year, int month) {
		...
		return findOrCreatePeriod(year, month, null, null, dateStart, dateEnd, domain,
	...
	private Period findOrCreatePeriodPerYear(Domain domain, int year) {
		...
		return findOrCreatePeriod(year, null, null, null, dateStart, dateEnd, domain,
				Period.GRANULARITY_YEAR);
	}

	@Transactional(propagation = Propagation.REQUIRED)
	public List<Period> findForUpdate(List<Long> ids) {
		return periodRepository.findForUpdate(ids);
	}
	...
	public Period findOrCreatePeriodByOffset(int offset, Domain domain, int granularity) {
		...
		log.debug("findOrCreatePeriodByOffset " + date + " domain " + domain + " granularity " + granularity
				+ " offset " + offset);
	...
	@Retryable(maxAttempts = 10, backoff = @Backoff(delay = 10, maxDelay = 100, random = true))
	private Period createPeriod(Integer year, Integer month, Integer day, Integer week, DateTime dateStart,
	        DateTime dateEnd, Domain domain, int granularity) {
		Period period = periodRepository.findByDomainAndYearAndMonthAndDayAndWeekAndGranularity(domain, year, month,
	...
	@Retryable(maxAttempts = 10, backoff = @Backoff(delay = 10, maxDelay = 100, random = true))
	private Period findOrCreatePeriod(Integer year, Integer month, Integer day, Integer week, DateTime dateStart,
	        DateTime dateEnd, Domain domain, int granularity) {
		if (day == null) day = -1;
		...
		log.debug("findOrCreatePeriod year " + year + " month " + month + " day " + day + " week " + week
				+ " dateStart " + dateStart.toString() + " dateEnd " + dateEnd.toString() + " domain "
		...
		if (period == null) {
			cacheMiss = true;
			period = periodRepository.findByDomainAndYearAndMonthAndDayAndWeekAndGranularity(domain, year, month, day,
					week, granularity);
		}
		
		if (period == null) {
			log.info("findOrCreatePeriod creating period year " + year + " month " + month + " day " + day + " week "
					+ week + " dateStart " + dateStart.toString() + " dateEnd " + dateEnd.toString() + " domain "
					+ domain.getName() + " granularity " + granularity);
			period = createPeriod(year, month, day, week, dateStart, dateEnd, domain, granularity);
			log.info("findOrCreatePeriod created period year " + year + " month " + month + " day " + day + " week "
					+ week + " dateStart " + dateStart.toString() + " dateEnd " + dateEnd.toString() + " domain "
					+ domain.getName() + " granularity " + granularity);
		}
	...
	private Period cacheGet(Integer year, Integer month, Integer day, Integer week, DateTime dateStart,
			DateTime dateEnd, Domain domain, int granularity) {
		String cacheKey = 
				year + "_" + 
				month + "_" + 
