  ...
  >
    <template v-slot:activator="{ on, attrs }">
      <v-btn
          ...
          dark
      >
        {{ allTagsAreSelected && tags.length ? translate("TYPE.ALL")  : tagsAreSelectedText }}
        <v-icon
            dense
            right
            color="white"
...
@Component
export default class TypeDropDownMenu extends Vue {
  ...
  get tagsAreSelectedText(): string {
    ...
    if(list.length === 0 || !this.tags.length ) {
      return  this.translate("TYPE.NO")
    }
    else if(list.length > 1 ) {
      return list.length + ' ' + this.translate("TYPE.MANY")
    }
    return list.length + ' ' + this.translate("TYPE.ONE")
  }
