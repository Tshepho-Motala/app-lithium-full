...
@Slf4j
@Service
public class PubSubGoogleServiceMarketingPreferencesImpl implements PubSubGoogleService{
    @Autowired
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = "service.google.pub-sub.marketing-preferences", durable = "true"),
                    exchange = @Exchange(value = "service.google.pub-sub.marketing-preferences", durable = "true", type = "topic"),
                    key = "service.google.pub-sub.marketing-preferences"
            )
    )
    public void googlePubSubQueueListener(String request) throws RabbitConsumerErrorException {
        try {
            ...
            if (!configService.isChannelActivated(message.getDomainName(), DataType.MARKETING_PREFERENCES.name())) {
                log.debug("try to consume income rabbit message " + request + "but pub-sub channel is not activated, retry");
                log.warn("Pub-sub channel is not activated: " + DataType.MARKETING_PREFERENCES.name());
                throw new RabbitConsumerErrorException("pub-sub service is not activated");
            }
            sendMessage(request, message.getDomainName());
        } catch (IOException e) {
            log.error("Can't proceed incoming RabbitMQ message, skipped " + request + "because exception:" + e.getMessage());
        }
    }
    ...
    public void sendMessage(String incomeMessage, String domainName) throws IOException {
        ...
        if (domainName != null) {
            ...
            try {
                publisher = publishersInitService.getPublisher(domainName, DataType.MARKETING_PREFERENCES);
                    ...
                    ApiFutures.addCallback(
                            ...
                            new ApiFutureCallback<String>() {
                                ...
                                public void onFailure(Throwable throwable) {
                                    ...
                                    try {
                                        pubSubStream.processMarketingPreferences(new ObjectMapper().readValue(incomeMessage, PubSubMessage.class));
                                    } catch (PubSubInternalErrorException | JsonProcessingException ex) {
                                        log.error("Unable to requeue failed pub-sub message on processMarketingPreferences | incomeMessage: {}", incomeMessage);
                                    }
                                }

                                @Override
                                public void onSuccess(String messageId) {
                                    log.info("Published pub-sub message to channel: " + DataType.MARKETING_PREFERENCES.getChannelName() + " with ID: " + messageId);
                                    log.debug("Published pub-sub message to channel: " + DataType.MARKETING_PREFERENCES.getChannelName() + " with ID: " + messageId + "message: " + incomeMessage);
                                }
                            },
                            taskExecutor
                    );
