...
@Slf4j
@Service
public class PubSubGoogleServiceAccountLinkChangeImpl implements PubSubGoogleService {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = "service.google.pub-sub.link", durable = "true"),
                    exchange = @Exchange(value = "service.google.pub-sub.link", durable = "true", type = "topic"),
                    key = "service.google.pub-sub.link"
            )
    )
    public void googlePubSubQueueListener(String request) throws RabbitConsumerErrorException {
        try {
            ...
            if (!configService.isChannelActivated(message.getDomainName(), DataType.ACCOUNT_LINK_CHANGES.name())) {
                log.debug("try to consume income rabbit message " + request + "but pub-sub channel is not activated, retry");
                log.warn("Pub-sub channel is not activated: " + DataType.ACCOUNT_LINK_CHANGES.name());
    ...
    public void sendMessage(String incomeMessage, String domainName) throws IOException {
        ...
        if (domainName != null) {
            ...
            try {
                publisher = publishersInitService.getPublisher(domainName, DataType.ACCOUNT_LINK_CHANGES);
                    ...
                    ApiFutures.addCallback(
                            ...
                            new ApiFutureCallback<String>() {
                                ...
                                public void onFailure(Throwable throwable) {
                                    ...
                                    try {
                                        pubSubStream.processAccountLinkChange(new ObjectMapper().readValue(incomeMessage, PubSubMessage.class));
                                    } catch (PubSubInternalErrorException | JsonProcessingException ex) {
                                        log.error("Unable to requeue failed pub-sub message on processAccountLinkChange | incomeMessage: {}", incomeMessage);
                                    }
                                }

                                @Override
                                public void onSuccess(String messageId) {
                                    log.info("Published pub-sub message to channel: " + DataType.ACCOUNT_LINK_CHANGES.getChannelName() + " with ID: " + messageId);
                                    log.debug("Published pub-sub message to channel: " + DataType.ACCOUNT_LINK_CHANGES.getChannelName() + " with ID: " + messageId + "message: " + incomeMessage);
                                }
                            },
                            taskExecutor
                    );
