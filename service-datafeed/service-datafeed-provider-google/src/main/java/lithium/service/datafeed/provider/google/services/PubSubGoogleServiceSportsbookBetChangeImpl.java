...
@Slf4j
@Service
public class PubSubGoogleServiceSportsbookBetChangeImpl implements PubSubGoogleService {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = "service.google.pub-sub.sportsbook", durable = "true"),
                    exchange = @Exchange(value = "service.google.pub-sub.sportsbook", durable = "true", type = "topic"),
                    key = "service.google.pub-sub.sportsbook"
            )
    )
    public void googlePubSubQueueListener(String request) throws RabbitConsumerErrorException {
        try {
            ...
            if (!configService.isChannelActivated(message.getDomainName(), DataType.SPORTSBOOK_TRANSACTIONS.name())) {
                log.debug("try to consume income rabbit message " + request + "but pub-sub channel is not activated, retry");
                log.warn("Pub-sub channel is not activated: " + DataType.SPORTSBOOK_TRANSACTIONS.name());
    ...
    public void sendMessage(String incomeMessage, String domainName) throws IOException {
        ...
        if (domainName != null) {
            ...
            try {
                publisher = publishersInitService.getPublisher(domainName, DataType.SPORTSBOOK_TRANSACTIONS);
                    ...
                    ApiFutures.addCallback(
                            ...
                            new ApiFutureCallback<String>() {
                                ...
                                public void onFailure(Throwable throwable) {
                                    ...
                                    try {
                                        pubSubStream.processSportsBookChange(new ObjectMapper().readValue(incomeMessage, PubSubMessage.class));
                                    } catch (PubSubInternalErrorException | JsonProcessingException ex) {
                                        log.error("Unable to requeue failed pub-sub message on processSportsBookChange | incomeMessage: {}", incomeMessage);
                                    }
                                }

                                @Override
                                public void onSuccess(String messageId) {
                                    log.info("Published pub-sub message to channel: " + DataType.SPORTSBOOK_TRANSACTIONS.getChannelName() + " with ID: " + messageId);
                                    log.debug("Published pub-sub message to channel: " + DataType.SPORTSBOOK_TRANSACTIONS.getChannelName() + " with ID: " + messageId + "message: " + incomeMessage);
                                }
                            },
                            taskExecutor
                    );
