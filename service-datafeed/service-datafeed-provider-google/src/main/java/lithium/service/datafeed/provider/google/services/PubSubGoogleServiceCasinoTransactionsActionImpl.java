...
@Slf4j
@Service
public class PubSubGoogleServiceCasinoTransactionsActionImpl implements PubSubGoogleService {
    ...
    @RabbitListener(
            bindings = @QueueBinding(
                    value = @Queue(value = "service.google.pub-sub.casino", durable = "true"),
                    exchange = @Exchange(value = "service.google.pub-sub.casino", durable = "true", type = "topic"),
                    key = "service.google.pub-sub.casino"
            )
    )
    public void googlePubSubQueueListener(String request) throws RabbitConsumerErrorException {
        try {
            ...
            if (!configService.isChannelActivated(message.getDomainName(), DataType.CASINO_TRANSACTIONS.name())) {
                log.debug("try to consume income rabbit message " + request + "but pub-sub channel is not activated, retry");
                log.warn("Pub-sub channel is not activated: " + DataType.CASINO_TRANSACTIONS.name());
    ...
    public void sendMessage(String incomeMessage, String domainName) throws IOException {
        ...
        if (domainName != null) {
            try {
                publisher = publishersInitService.getPublisher(domainName, DataType.CASINO_TRANSACTIONS);
                    ...
                    ApiFutures.addCallback(
                            ...
                            new ApiFutureCallback<String>() {
                                ...
                                public void onFailure(Throwable throwable) {
                                    ...
                                    try {
                                        pubSubStream.processCasinoChange(new ObjectMapper().readValue(incomeMessage, PubSubMessage.class));
                                    } catch (PubSubInternalErrorException | JsonProcessingException ex) {
                                        log.error("Unable to requeue failed pub-sub message on processCasinoChange | incomeMessage: {}", incomeMessage);
                                    }
                                }

                                @Override
                                public void onSuccess(String messageId) {
                                    log.info("Published pub-sub message to channel: " + DataType.CASINO_TRANSACTIONS.getChannelName()+ " with ID: " + messageId);
                                    log.debug("Published pub-sub message to channel: " + DataType.CASINO_TRANSACTIONS.getChannelName()+ " with ID: " + messageId + "message: " + incomeMessage);
                                }
                            },
                            taskExecutor
                    );
