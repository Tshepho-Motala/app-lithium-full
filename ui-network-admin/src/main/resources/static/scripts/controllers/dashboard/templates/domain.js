...
.controller('TemplatesDomainController', ["$translate", "$scope", "$userService", "$stateParams", "$state", "$filter", "$rootScope",
	function($translate, $scope, $userService, $stateParams, $state, $filter,  $rootScope) {
		var controller = this;

		controller.selectedDomain = null
		controller.textTitle = 'UI_NETWORK_ADMIN.PAGE_HEADER.CMS_TEMPLATES.TITLE'
		controller.textDescr = 'UI_NETWORK_ADMIN.PAGE_HEADER.CMS_TEMPLATES.DESCRIPTION'
		
		controller.tabs = [
			{ name: "dashboard.templates.domain.list", title: "UI_NETWORK_ADMIN.TEMPLATES.TAB.LIST", roles: "ADMIN,TEMPLATES_MANAGE,TEMPLATES_VIEW,TEMPLATES_ADD,TEMPLATES_EDIT" }
		];
