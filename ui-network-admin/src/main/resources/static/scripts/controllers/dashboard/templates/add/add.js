...
angular.module('lithium')
	.controller('TemplatesAddController', ["domainName", "$uibModalInstance", "TemplatesRest", "notify", "errors", "$q",
	function(domainName, $uibModalInstance, rest, notify, errors, $q) {
				...
				asyncValidators: {
					nameUnique: {
						expression: function($viewValue, $modelValue, scope) {
							var success = false;
							return rest.findByNameAndLangAndDomainName(encodeURIComponent($viewValue), controller.model.lang, domainName).then(function(emailTemplate) {
		...
		controller.onSubmit = function() {
			...
			rest.add(controller.model, domainName).then(function(response) {
				notify.success("UI_NETWORK_ADMIN.TEMPLATES.SUCCESS.ADD");
				$uibModalInstance.close(response);
			}).catch(function(error) {
				notify.error("UI_NETWORK_ADMIN.TEMPLATES.ERRORS.ADD");
