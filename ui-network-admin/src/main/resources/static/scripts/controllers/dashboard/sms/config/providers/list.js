...
.controller('SMSProvidersListController',
["rest-sms", "domainProviders", "$scope", "$stateParams", "$uibModal", "notify", "errors", "$filter",
function(smsRest, domainProviders, $scope, $stateParams, $uibModal, notify, errors, $filter) {
	...
	controller.addDomainProvider = function() {
		var modalInstance = $uibModal.open({
			...
			templateUrl: 'scripts/controllers/dashboard/sms/config/providers/addprovider.html',
	...
	controller.saveDomainProviderOrder = function() {
		...
		smsRest.domainProviderUpdateMultiple(controller.domainProviders).then(function(dps) {
			controller.domainProviders = dps.plain();
			angular.extend(controller.domainProviders, dps.plain());
			notify.success("UI_NETWORK_ADMIN.SMS.PROVIDERS.EDIT.ORDERCHANGEDSUCCESS");
	...
	controller.editDomainProvider = function(domainProvider) {
		var modalInstance = $uibModal.open({
			...
			ariaLabelledBy: 'modal-title',
			ariaDescribedBy: 'modal-body',
			templateUrl: 'scripts/controllers/dashboard/sms/config/providers/editprovider.html',
			...
			resolve: {
				...
				loadMyFiles:function($ocLazyLoad) {
					return $ocLazyLoad.load({
						name:'lithium',
						files: [
							'scripts/controllers/dashboard/sms/config/providers/editprovider.js'
						]
					})
				}
			}
	...
	controller.toggleDomainProvider = function(domainProvider) {
		...
		smsRest.domainProviderUpdateMultiple(controller.domainProviders).then(function(dps) {
			controller.domainProviders = dps.plain();
			if (enabled) {
				notify.success("UI_NETWORK_ADMIN.SMS.PROVIDERS.EDIT.ENABLED");
			} else {
				notify.success("UI_NETWORK_ADMIN.SMS.PROVIDERS.EDIT.DISABLED");
			}
		}).catch(function(error) {
			errors.catch("", false)(error)
		}).then(function() {
	...
	controller.deleteDomainProvider = function(domainProvider) {
		var modalInstance = $uibModal.open({
			...
			ariaLabelledBy: 'modal-title',
			ariaDescribedBy: 'modal-body',
			templateUrl: 'scripts/controllers/dashboard/sms/config/providers/deleteprovider.html',
			...
			resolve: {
				...
				loadMyFiles:function($ocLazyLoad) {
					return $ocLazyLoad.load({
						name:'lithium',
						files: [
							'scripts/controllers/dashboard/sms/config/providers/deleteprovider.js'
						]
					})
				}
			}
	...
	controller.editProperties = function(domainProvider) {
		var modalInstance = $uibModal.open({
			...
			ariaLabelledBy: 'modal-title',
			ariaDescribedBy: 'modal-body',
			templateUrl: 'scripts/controllers/dashboard/sms/config/providers/properties.html',
			...
			resolve: {
				...
				loadMyFiles:function($ocLazyLoad) {
					return $ocLazyLoad.load({
						name:'lithium',
						files: [
							'scripts/controllers/dashboard/sms/config/providers/properties.js'
						]
					})
				}
			}
	...
	controller.reload = function() {
		smsRest.domainProviders($stateParams.domainName).then(function(domainProviders) {
	...
	controller.treeOptions = {
		accept: function(sourceNodeScope, destNodesScope, destIndex) {
			if (sourceNodeScope.$treeScope != destNodesScope.$treeScope) {
				...
				notify.warning("UI_NETWORK_ADMIN.SMS.PROVIDERS.EDIT.ORDERCHANGED", {ttl: 30000});
				return true;
			}
		}
	};
