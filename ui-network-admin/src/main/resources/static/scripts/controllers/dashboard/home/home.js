...
angular.module('lithium')
	.controller('home', ["$userService", "$http", "$translate", "$log", "$scope", "$timeout", "rest-domain", "$rootScope", "$interval", "$stateParams",
	function($userService, $http, $translate, $log, $scope, $timeout, domainRest, $rootScope, $interval, $stateParams) {
		var controller = this;
		...
		controller.textTitle = 'UI_NETWORK_ADMIN.PAGE_HEADER.DASHBOARD.TITLE'
		controller.textDescr = 'UI_NETWORK_ADMIN.PAGE_HEADER.DASHBOARD.DESCRIPTION'
...
//				},
		...
		$rootScope.provide.pageHeaderProvider.getDomainsList = () => {
			return $userService.playerDomainsWithAnyRole(["ADMIN", "DASHBOARD"])
		}
		...
		$rootScope.provide.pageHeaderProvider.domainSelect = ( item, isAlreadyChecked ) =>  {
			...
			if(controller.selectedDomains && controller.selectedDomains.length > 0) {
				domainRest.findByName(controller.selectedDomain).then(function(domain) {
					controller.currency = {
						code: domain.currency,
						symbol: domain.currencySymbol,
						format: domain.currencySymbol+' '+"##.00"
					};
