...
.controller('PushmsgProvidersListController',
["rest-pushmsg", "domainProviders", "$scope", "$stateParams", "$uibModal", "notify", "errors", "$filter",
function(pushmsgRest, domainProviders, $scope, $stateParams, $uibModal, notify, errors, $filter) {
	...
	controller.addDomainProvider = function() {
		var modalInstance = $uibModal.open({
			...
			templateUrl: 'scripts/controllers/dashboard/pushmsg/config/providers/addprovider.html',
	...
	controller.saveDomainProviderOrder = function() {
		...
		pushmsgRest.domainProviderUpdateMultiple(controller.domainProviders).then(function(dps) {
			controller.domainProviders = dps.plain();
			angular.extend(controller.domainProviders, dps.plain());
			notify.success("UI_NETWORK_ADMIN.PUSHMSG.PROVIDERS.EDIT.ORDERCHANGEDSUCCESS");
	...
	controller.editDomainProvider = function(domainProvider) {
		var modalInstance = $uibModal.open({
			...
			ariaLabelledBy: 'modal-title',
			ariaDescribedBy: 'modal-body',
			templateUrl: 'scripts/controllers/dashboard/pushmsg/config/providers/editprovider.html',
			...
			resolve: {
				...
				loadMyFiles:function($ocLazyLoad) {
					return $ocLazyLoad.load({
						name:'lithium',
						files: [
							'scripts/controllers/dashboard/pushmsg/config/providers/editprovider.js'
						]
					})
				}
			}
	...
	controller.toggleDomainProvider = function(domainProvider) {
		...
		pushmsgRest.domainProviderUpdateMultiple(controller.domainProviders).then(function(dps) {
			controller.domainProviders = dps.plain();
			if (enabled) {
				notify.success("UI_NETWORK_ADMIN.PUSHMSG.PROVIDERS.EDIT.ENABLED");
			} else {
				notify.success("UI_NETWORK_ADMIN.PUSHMSG.PROVIDERS.EDIT.DISABLED");
			}
		}).catch(function(error) {
			errors.catch("", false)(error)
		}).then(function() {
	...
	controller.deleteDomainProvider = function(domainProvider) {
		var modalInstance = $uibModal.open({
			...
			ariaLabelledBy: 'modal-title',
			ariaDescribedBy: 'modal-body',
			templateUrl: 'scripts/controllers/dashboard/pushmsg/config/providers/deleteprovider.html',
			...
			resolve: {
				...
				loadMyFiles:function($ocLazyLoad) {
					return $ocLazyLoad.load({
						name:'lithium',
						files: [
							'scripts/controllers/dashboard/pushmsg/config/providers/deleteprovider.js'
						]
					})
				}
			}
	...
	controller.editProperties = function(domainProvider) {
		var modalInstance = $uibModal.open({
			...
			ariaLabelledBy: 'modal-title',
			ariaDescribedBy: 'modal-body',
			templateUrl: 'scripts/controllers/dashboard/pushmsg/config/providers/properties.html',
			...
			resolve: {
				...
				loadMyFiles:function($ocLazyLoad) {
					return $ocLazyLoad.load({
						name:'lithium',
						files: [
							'scripts/controllers/dashboard/pushmsg/config/providers/properties.js'
						]
					})
				}
			}
	...
	controller.reload = function() {
		pushmsgRest.domainProviders($stateParams.domainName).then(function(domainProviders) {
	...
	controller.treeOptions = {
		accept: function(sourceNodeScope, destNodesScope, destIndex) {
			if (sourceNodeScope.$treeScope != destNodesScope.$treeScope) {
				...
				notify.warning("UI_NETWORK_ADMIN.PUSHMSG.PROVIDERS.EDIT.ORDERCHANGED", {ttl: 30000});
				return true;
			}
		}
	};
