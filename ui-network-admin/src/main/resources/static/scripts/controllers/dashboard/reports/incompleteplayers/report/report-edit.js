...
angular.module('lithium').controller('ReportIncompletePlayersEdit', ['$scope', '$state', '$userService', 'bsLoadingOverlayService', '$uibModal', 'notify', 'report', 'ReportIncompletePlayersRest', '$translate',
	function($scope, $state, $userService, bsLoadingOverlayService, $uibModal, notify, report, ReportIncompletePlayersRest, $translate) {
		let controller = this;
		...
		controller.init = function() {
			...
			ReportIncompletePlayersRest.getFilters(report.id, true).then(function(result) {
			...
			ReportIncompletePlayersRest.getActions(report.id, true).then(function(result) {
		...
		controller.onSubmit = function() {
			...
			ReportIncompletePlayersRest.editPost(report.id, 'commit', controller.model).then(function(response) {
				if (response._successful) {
					notify.success("UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.NOTIFICATION.EDIT_SAVE_SUCCESS");
					$state.go("^.report", { reportId:response.id });
				} else {
					notify.error("UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.NOTIFICATION.EDIT_SAVE_FAIL");
				}
			});
		}
		...
		controller.onContinue = function() {
			...
			ReportIncompletePlayersRest.editPost(report.id, 'continue', controller.model).then(function(response) {
				if (response._successful) {
					notify.success("UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.NOTIFICATION.EDIT_SAVE_SUCCESS");
					$state.go("^.report", { reportId:response.id });
				} else {
		...
		controller.onCancel = function() {
			ReportIncompletePlayersRest.editPost(report.id, 'cancel', controller.model).then(function(response) {
				if (response._successful) {
