...
angular.module('lithium')
	.controller('ReportIncompletePlayers', ["$scope", "notify", "$state", "$rootScope", "$dt", "$translate", "$uibModal", "bsLoadingOverlayService", "ReportIncompletePlayersRest",
	function($scope, notify, $state, $rootScope, $dt, $translate, $uibModal, bsLoadingOverlayService, ReportIncompletePlayersRest) {
		let controller = this;
		...
		ReportIncompletePlayersRest.view(reportId).then(function(result) {
			console.log(result);
			$scope.report = result;
		...
		ReportIncompletePlayersRest.getFilters(reportId, false).then(function(result) {
		...
		ReportIncompletePlayersRest.getActions(reportId, false).then(function(result) {
			$scope.reportActions = result;
			bsLoadingOverlayService.stop({ referenceId: 'player-report-actions' });
		});

		let baseUrl = "services/service-report-incomplete-players/report/players/" + reportId + "/runs/table?1=1";
		...
		.column($dt.linkscolumn("", [{
				permission: "report_incomplete_players",
				permissionType:"any", 
		...
		controller.getFilterOperator = function(filter) {
			if (filter.field === 'playerCreatedDate') {
