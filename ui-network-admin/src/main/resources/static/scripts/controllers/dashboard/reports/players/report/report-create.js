...
angular.module('lithium')
	.controller('ReportPlayersCreate', ["$scope", "notify", "$state", "$rootScope", "$dt", "$translate", "bsLoadingOverlayService", "ReportPlayersRest", "$userService", "$uibModal",
	function($scope, notify, $state, $rootScope, $dt, $translate, bsLoadingOverlayService, ReportPlayersRest, $userService, $uibModal) {
		let controller = this;
		...
		controller.model.filters.push({field: "playerStatus", operator: "equalTo", value: "Enabled"});
					...
					[
						...
						},
						{
							...
							templateOptions : {
								...
								placeholder : '',
								options : [
											{ name: $translate.instant('UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.SECTION_LABELS.IMMEDIATE'), value: 1 },
											{ name: $translate.instant('UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.SECTION_LABELS.DATE_AND_TIME'), value: 2 },
											{ name: $translate.instant('UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.SECTION_LABELS.RECURRING'), value: 3 }
										]
							},
							expressionProperties: {
		...
		controller.onSubmit = function() {
			...
			ReportPlayersRest.create(controller.model).then(function(response) {
				if (response._successful) {
					...
					notify.error('UI_NETWORK_ADMIN.REPORTS.PLAYERS.FIELDS.NOTIFICATION.CREATION_SUCCESS');
				}
			});
		}
	}
