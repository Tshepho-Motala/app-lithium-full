...
angular.module('lithium').controller('AutoWithdrawalRulesetNameModal', ["ruleset", "errors", "$scope", "notify", "$uibModalInstance", "bsLoadingOverlayService", "AutoWithdrawalRulesetRest",
    function (ruleset, errors, $scope, notify, $uibModalInstance, bsLoadingOverlayService, rest) {
        ...
        controller.onSubmit = function() {
            ...
            rest.autoWithdrawalRulesetChangeName(ruleset.domain.name, ruleset.id, controller.model.name)
                .then(function (response) {
                    if (response._status !== 0) {
                        notify.error(response._message);
                    } else {
                        notify.success("UI_NETWORK_ADMIN.CASHIER.AUTOWITHDRAWALS.RULESETS.CHANGENAME.SUCCESS");
                        $uibModalInstance.close(response);
                    }
                }).catch(
                errors.catch("UI_NETWORK_ADMIN.CASHIER.AUTOWITHDRAWALS.RULESETS.CHANGENAME.ERROR", false)
            ).finally(function () {
