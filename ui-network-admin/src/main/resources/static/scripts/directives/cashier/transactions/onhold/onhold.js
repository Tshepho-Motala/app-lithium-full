...
function ($uibModalInstance, ownerGuid, cashierRest, notify, errors, bsLoadingOverlayService) {
	...
	controller.submit = function() {
		...
		cashierRest.withdrawalOnHold(ownerGuid, controller.model.comment).then(function(response) {
			if (response != undefined && response >=0 ) {
				notify.success("On hold command was sent successfully for "+ response+" transactions");
			} else {
				errors.catch("UI_NETWORK_ADMIN.PLAYER.WITHDRAWAL_HOLD.ERROR", false)
			}
