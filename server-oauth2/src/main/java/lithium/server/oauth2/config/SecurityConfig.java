package lithium.server.oauth2.config;
		...
		try {
			if ((response != null) && (!response.isSuccessful())) {
				...
				if (response.getStatus().id().equals(Response.Status.LOGIN_RESTRICTED.id())) {
					throw new Status460LoginRestrictedException(LoginError.RESTRICTED.getResponseMessageLocal(messageSource, domainName, response.getMessage(), response.getMessage()));
				}
				...
				if (response.getStatus().id().equals(Status.ACCOUNT_UPGRADE_REQUIRED.id())) {
					...
					CustomOAuthException customOAuthException = new CustomOAuthException("430", exception);
					if (response.getData() != null) {
						customOAuthException.addAdditionalInformation("email", response.getData().getEmail());
						customOAuthException.addAdditionalInformation("uuid", response.getData().getApiToken());

						if (!ObjectUtils.isEmpty(response.getData().getFirstName())) {
							customOAuthException.addAdditionalInformation("firstName", response.getData().getFirstName());
						}

						if (!ObjectUtils.isEmpty(response.getData().getLastName())) {
							customOAuthException.addAdditionalInformation("lastName", response.getData().getLastName());
						}
						if (!ObjectUtils.isEmpty(response.getData().getDobDay())) {
							customOAuthException.addAdditionalInformation("dobDay", response.getData().getDobDay().toString());
						}
						if (!ObjectUtils.isEmpty(response.getData().getDobMonth())) {
							customOAuthException.addAdditionalInformation("dobMonth", response.getData().getDobMonth().toString());
						}
						if (!ObjectUtils.isEmpty(response.getData().getDobYear())) {
							customOAuthException.addAdditionalInformation("dobYear", response.getData().getDobYear().toString());
						}
						if (!ObjectUtils.isEmpty(response.getData().getEmailOptOut())) {
							customOAuthException.addAdditionalInformation("emailOptOut", String.valueOf(response.getData().getEmailOptOut()));
						}
						if (!ObjectUtils.isEmpty(response.getData().getCellphoneNumber())) {
							customOAuthException.addAdditionalInformation("cellphoneNumber", String.valueOf(response.getData().getCellphoneNumber()));
						}
						if (!ObjectUtils.isEmpty(response.getData().getResidentialAddress())) {
							if (!ObjectUtils.isEmpty(response.getData().getResidentialAddress().getAddressLine1())) {
								customOAuthException.addAdditionalInformation("addressLine1", response.getData().getResidentialAddress().getAddressLine1());
							}
							if (!ObjectUtils.isEmpty(response.getData().getResidentialAddress().getCity())) {
								customOAuthException.addAdditionalInformation("city", response.getData().getResidentialAddress().getCity());
							}
							if (!ObjectUtils.isEmpty(response.getData().getResidentialAddress().getPostalCode())) {
								customOAuthException.addAdditionalInformation("postalCode", response.getData().getResidentialAddress().getPostalCode());
							}
							if (!ObjectUtils.isEmpty(response.getData().getResidentialAddress().getCountryCode())) {
								customOAuthException.addAdditionalInformation("countryCode", response.getData().getResidentialAddress().getCountryCode());
							}
						}
				...
				if (response.getStatus().id().equals(Status.MUTUAL_EXCLUSIVE_DOMAIN_EXIST.id())) {
					throw new Status432UserExistsInAnotherExclusiveDomainException(response.getMessage());
				}
			...
			TokenService.storeInThread(TokenService.TL_DATA_DOMAIN, domain);
				...
				Status492ExcessiveFailedLoginBlockException |
				Status496PlayerCoolingOffException |
				Status432UserExistsInAnotherExclusiveDomainException e
		) {
		...
		return new AuthenticationProvider() {
			...
			public Authentication authenticate(Authentication authentication) throws AuthenticationException {
				...
				try {
					...
					// origin-pass is used to block players to only use the players login endpoint and backoffice to only use the backoffice login endpoint
					if (detailsObject instanceof Map) {
						Map<String, String> details = (Map<String, String>) detailsObject;
						String originPass = details.get("origin-pass");

						if (originPass != null && !originPass.isEmpty()) {
							extraParameters.put("origin-pass", originPass);
						}
					}
					...
					//Send authentication data through the body instead of querystring
					Response<User> response = userClient.auth(authRequest);
