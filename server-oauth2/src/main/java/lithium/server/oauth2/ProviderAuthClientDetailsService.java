...
import lithium.systemauth.SystemAuthConfigurationProperties;
...
public class ProviderAuthClientDetailsService implements ClientDetailsService {
	private static final String NO_OP_PASSWORD_ENCODER_ID = "{noop}";
	...
	public ClientDetails loadClientByClientId(String clientId) throws ClientRegistrationException {
		...
		if (clientId.equalsIgnoreCase("system")) {
			client = client.toBuilder()
				.clientSecret(getNoEncoderPassword(properties.getPassword()))
			...
			try {
				...
				client = client.toBuilder()
					.clientSecret(getNoEncoderPassword(pac.getPassword()))
					.resourceId("oauth2-resource")
...
//		base.setAdditionalInformation(client.getAdditionalInformation());
	...
	public static String getNoEncoderPassword(String password){
		return NO_OP_PASSWORD_ENCODER_ID + password;
	}
}