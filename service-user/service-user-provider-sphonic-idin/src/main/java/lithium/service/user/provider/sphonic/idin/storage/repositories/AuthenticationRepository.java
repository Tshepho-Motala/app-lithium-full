...
public interface AuthenticationRepository extends SphonicAuthenticationRepository<Authentication, Long> {
	@CacheEvict(cacheNames = "lithium.service.user.provider.sphonic.idin.storage.entities.Authentication",
			key = "#root.args[0].getDomain().getName()")
	@Override
	<S extends Authentication> S save(S entity);

	@Cacheable(cacheNames="lithium.service.user.provider.sphonic.idin.storage.entities.Authentication",
