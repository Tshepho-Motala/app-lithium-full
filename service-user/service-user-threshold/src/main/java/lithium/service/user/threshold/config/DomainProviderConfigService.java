...
@Service
@Slf4j
public class DomainProviderConfigService {

  @Autowired
  private LithiumServiceClientFactory services;

  public DomainProviderConfig getConfig(String providerName, String domainName)
  throws Status512ProviderNotConfiguredException
  {
    ...
    DomainProviderConfig config = new DomainProviderConfig(); //external system id = providerId as stored in domain config
    for (ProviderProperty p: pp.getData()) {
        if (p.getName().equalsIgnoreCase(ProviderConfigProperties.EXTREME_PUSH_API_URL.getValue())) {
            config.setExtremePushApiUrl(p.getValue());
        }
        if (p.getName().equalsIgnoreCase(ProviderConfigProperties.EXTREME_PUSH_APP_TOKEN.getValue())) {
            config.setExtremePushAppToken(p.getValue());
        }
    }
