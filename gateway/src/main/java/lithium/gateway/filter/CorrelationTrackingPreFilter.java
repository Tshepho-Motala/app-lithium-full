package lithium.gateway.filter;

import static java.lang.String.format;
import static lithium.gateway.filter.FilterOrderType.PRE;
...
public class CorrelationTrackingPreFilter implements GlobalFilter, Ordered {
  ...
  public Mono<Void> filter (ServerWebExchange exchange, GatewayFilterChain chain) {
    ...
    if (hasCorrelationId(header)) {
      ...
      request = exchange.getRequest().mutate().header(CORRELATION_ID, generateCorrelationId()).build();
